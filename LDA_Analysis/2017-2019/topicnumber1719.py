...
def load_data(years):
    all_docs = []
    for year in years:
        with open(f'../../data/tokenized_data/2017-2019/tokenized_{year}.json', 'r', encoding='utf-8') as f:
            docs = json.load(f)
...
def plot_metrics(perplexity_values, coherence_values, start=2, limit=11, step=1):
    ...
    plt.title('主题数与模型评估指标关系 (2017-2019)', fontsize=16, pad=20)
    plt.tight_layout()
    plt.savefig('../../data/tokenized_data/2017-2019/lda_metrics.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    # 保存结果到文件
    with open('../../data/tokenized_data/2017-2019/lda_topics.txt', 'w', encoding='utf-8') as f:
        f.write(f"主题数: 5\n\n")
        for topic_id in range(5):
    ...
    plt.bar(range(1, 6), topic_counts)
    plt.xlabel('主题编号')
    plt.ylabel('文章数量')
    plt.title('各主题文章数量分布')
    ...
    plt.tight_layout()
    plt.savefig('../../data/tokenized_data/2017-2019/topic_distribution.png', dpi=300)
    plt.show()
