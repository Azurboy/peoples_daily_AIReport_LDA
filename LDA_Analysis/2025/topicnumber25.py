...
def load_data(years):
    all_docs = []
    for year in years:
        with open(f'../../data/tokenized_data/2025/tokenized_{year}.json', 'r', encoding='utf-8') as f:
            docs = json.load(f)
...
# 计算困惑度和一致性
def compute_metrics(corpus, dictionary, texts, start=2, limit=10, step=1):
    perplexity_values = []
    coherence_values = []
    model_list = []
...
def plot_metrics(perplexity_values, coherence_values, start=2, limit=11, step=1):
    ...
    plt.title('主题数与模型评估指标关系 (2025-)', fontsize=16, pad=20)
    plt.tight_layout()
    plt.savefig('../../data/tokenized_data/2025/lda_metrics.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    # 计算不同主题数的评估指标
    start, limit, step = 2, 11, 1  # 评估2-20个主题
    ...
    # 保存结果到文件
    with open('../../data/tokenized_data/2025/lda_topics.txt', 'w', encoding='utf-8') as f:
        f.write("主题数: 6\n\n")
        for topic_id in range(6):
    ...
    plt.tight_layout()
    plt.savefig('../../data/tokenized_data/2025/topic_distribution.png', dpi=300)
    plt.show()
